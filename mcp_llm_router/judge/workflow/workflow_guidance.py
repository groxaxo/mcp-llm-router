...
from mcp_llm_router.judge.core.logging_config import get_logger
...
async def calculate_next_stage(
    ...
    try:
        ...
        from mcp_llm_router.judge.models import SystemVars
        ...
        from mcp_llm_router.judge.prompting.loader import (
        ...
        from mcp_llm_router.judge.models import JudgeCodingPlanUserVars
        ...
        from mcp_llm_router.judge.core.server_helpers import extract_json_from_response
...
async def _get_tool_descriptions() -> str:
    ...
    """
    try:
        # Access the MCP server instance used to register judge tools
        from mcp_llm_router.judge.server import get_registered_mcp

        mcp = get_registered_mcp()
        if mcp is None:
            raise RuntimeError("Judge tools are not registered with a FastMCP server.")
...
async def _get_available_tool_names() -> set[str]:
    ...
    """
    try:
        from mcp_llm_router.judge.server import get_registered_mcp  # Local import to avoid cycles
