...
class CleanFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if module_name.startswith("mcp_llm_router.judge."):
            module_name = module_name[len("mcp_llm_router.judge.") :]
...
def configure_application_loggers(level: int = logging.INFO) -> None:
    ...
    app_loggers = [
        "mcp_llm_router.judge.server",
        "mcp_llm_router.judge.core.server_helpers",
        "mcp_llm_router.judge.db.conversation_history_service",
        "mcp_llm_router.judge.db.providers.in_memory",
        "mcp_llm_router.judge.db.providers.sqlite_provider",
        "mcp_llm_router.judge.messaging",
        "mcp_llm_router.judge.llm",
        "mcp_llm_router.judge.config",
        "mcp_llm_router.judge.workflow.workflow_guidance",
        "mcp_llm_router.judge.tasks.manager",
    ]
...
class ContextAwareLogger:
    ...
    def __init__(self, name: str):
        ...
        if MCP_SDK_AVAILABLE:
            ...
            clean_name = name
            if name.startswith("mcp_llm_router.judge."):
                clean_name = name[len("mcp_llm_router.judge.") :]
            elif name == "__main__":
                clean_name = "server"
...
def log_startup_message(config: Any) -> None:
    ...
    """
    logger = get_logger("mcp_llm_router.judge.server")
